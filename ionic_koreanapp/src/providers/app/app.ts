...
export class AppProvider {
  ...
  addCompany(access_token, company_name, company_add, phone_no, working_days) {
    ...
    return new Promise(resolve => {
      this.http.get(apiKey + 'addCompany?access_token=' + access_token + '&company_name=' + company_name + '&company_add=' +
        company_add + '&phone_no=' + phone_no + '&working_days=' + working_days,
        { headers: { 'Access-Control-Allow-Origin': '*', 'Acess-Control-Allow-Methods': 'POST, GET, OPTIONS, PUT', 'Accept': 'application/json', 'content-type': 'application/json' } })
        .subscribe((res) => {
          resolve(res);
