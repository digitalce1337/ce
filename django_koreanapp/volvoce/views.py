...
def oauth(request):
    ...
    payload = "grant_type=authorization_code&client_id=98fc736440ad9232847e17e8b26eb7a6&redirect_uri=http%3A%2F%2F127.0.0.1%3A8000%2Foauth&code=" + str(code)
    headers = {
    ...
    if ("kaccount_email" in email):
        ...
        redirectloc = "/static/dobsignup.html?access_token="+str(access_token) + "&profileurl=" + str(profile_image_url) + "&nickname=" + str(nickname) + "&email=" + str(email)
        return HttpResponseRedirect(redirectloc)
    else:
        redirectloc = "/static/emailsignup.html?access_token="+str(access_token) + "&profileurl=" + str(profile_image_url) + "&nickname=" + str(nickname)
        return HttpResponseRedirect(redirectloc)
...
def signupdone(request):
    ...
    user = User.objects.create(email=email, name=name, dob=dob, profile_image_url=profile_image_url, access_token=access_token, companyName=company_name)
    user.save()
...
def addVeh(request):
    ...
    vehiclelist = VehicleList.objects.create(email=email,serial_no = serialNo,model_no=modelNo,purchase_date=purchasedate,desc=desc,manufacturer=manufacturer,vehicle_type=vtype, is_delete=0)
    vehiclelist.save()
    d = {}
    d['flag'] = 1
    return JsonResponse(d)
...
def GetVehList(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("SELECT vl.serial_no,vl.model_no,vl.purchase_date,vl.desc,vl.vehicle_type,vl.manufacturer,v.ImgUrl FROM volvoce.volvoce_vehiclelist as vl, volvoce.volvoce_vehicle as v where (vl.vehicle_type = v.Vtype and vl.email = %s and vl.is_delete=%s)", [email, '0'])
...
def getOwnerJoblist(request):
	...
	job_dateto = []
	with connection.cursor() as cursor:
		select_query = "SELECT j.description, j.date_from, j.date_to FROM volvoce.volvoce_joblist j INNER JOIN volvoce.volvoce_user u ON j.email=u.email WHERE u.access_token=\'%s\'" % (str(access_token))
...
def RemoveVeh(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("DELETE FROM volvoce.volvoce_vehiclelist WHERE volvoce_vehiclelist.email=%s and volvoce_vehiclelist.serial_no=%s and volvoce_vehiclelist.vehicle_type=%s",[email,sno,vtype])
        res = cursor.fetchone()
        print(res)
        d = {}
        d['flag'] = 1
...
def checkEmail(request):
	...
	try:
		with connection.cursor() as cursor:
			select_query = "SELECT email from volvoce.volvoce_user where email=\'%s\' and EXISTS (SELECT access_token from volvoce.volvoce_user where access_token=\'%s\')" % (str(email), str(access_token))
			cursor.execute(select_query)
			for row in cursor:
				if (row[0]):
					json_obj = {
...
def addOperator(request):
	...
	try:
		...
		if relationship:
			...
			relationship = OperatorList.objects.create(ownerEmail=owner_email, operatorEmail=operator_email, is_delete=0)
			relationship.save()
...
def getChartData(request):
	...
	try:
		with connection.cursor() as cursor:
			select_query = "SELECT j.JID, j.date_from, j.date_to from volvoce.volvoce_joblist j inner join volvoce.volvoce_user u ON j.email = u.email WHERE u.access_token = \'%s\';" % (str(access_token))
			print (select_query)
			cursor.execute(select_query)
			if cursor.rowcount == 0:
			...
			temp_from = []
			temp_to = []
			select_query = "SELECT j.jid, j.date_from, j.date_to from volvoce.volvoce_joblist j INNER JOIN volvoce.volvoce_operatorjob oj ON j.jid=oj.jid WHERE oj.email=\'%s\';" % (str(email))
			cursor.execute(select_query)
			if cursor.rowcount == 0:
...
def updownMaint(request):
    ...
    response = {}
    with connection.cursor() as cursor:
        cursor.execute("SELECT count(*) FROM volvoce.volvoce_maintenance WHERE email = %s and serial_no = %s and vtype = %s and date_to > NOW()",[email,sno,vtype])
        res = dictfetchall(cursor)
        response['upcomming'] = res[0]['count(*)']
        cursor.execute("SELECT count(*) FROM volvoce.volvoce_maintenance WHERE email = %s and serial_no = %s and vtype = %s and date_to < NOW()",[email,sno,vtype])
        res = dictfetchall(cursor)
        response['history'] = res[0]['count(*)']
        cursor.execute("SELECT date_to,datediff(date_to,NOW()) as delta,location FROM volvoce.volvoce_maintenance WHERE email = %s and serial_no = %s and vtype = %s and date_to < NOW() order by delta desc LIMIT 1",[email,sno,vtype])
        res = dictfetchall(cursor)
        try:
        ...
        cursor.execute("SELECT date_from,datediff(date_from,NOW()) as delta,location FROM volvoce.volvoce_maintenance WHERE email = %s and serial_no = %s and vtype = %s and date_to > NOW() order by delta asc LIMIT 1",[email,sno,vtype])
        res = dictfetchall(cursor)
        try:
...
def getMaintRec(request):
    ...
    vtype = request.GET['vtype']
    with connection.cursor() as cursor:
        cursor.execute("SELECT * FROM volvoce.volvoce_maintenance WHERE email = %s and serial_no = %s and vtype=%s",[email,sno,vtype])
        res = dictfetchall(cursor)
...
def removeMaintInternal(email,sno,vtype):
    with connection.cursor() as cursor:
        cursor.execute("DELETE FROM volvoce.volvoce_maintenance WHERE volvoce_maintenance.email=%s and volvoce_maintenance.serial_no=%s and volvoce_maintenance.vtype=%s",[email,sno,vtype])
        res = cursor.fetchone()
        cursor.close()
...
def retrieveVehicleUtil(request):
	...
	try:
		...
		return JsonResponse(json_obj)
	try:
		operator_job = OperatorJob.objects.filter(vehicle_serial_no=serial_no)
		for records in operator_job:
			jid.append(records.JID)
		jobmonth = JobList.objects.filter(JID__in=jid, date_from__gte=one_month_ago)
		for job in jobmonth:
			vehicle_month += 1
		total_job = JobList.objects.filter(email=owner_email, date_from__gte=one_month_ago)
		for job in total_job:
			month_total += 1
		jobyear = JobList.objects.filter(JID__in=jid, date_from__gte=one_year_ago)
		for job in jobyear:
			vehicle_year += 1
		total_job = JobList.objects.filter(email=owner_email, date_from__gte=one_year_ago)
		for job in total_job:
			year_total += 1
		json_obj = {
...
def retrieveMaintenance(request):
	...
	try:
		...
		return JsonResponse(json_obj)
	try:
		maintenance_records = Maintenance.objects.filter(email=email, serial_no=serial_no, date_to__gt=datetime.date(today.year, today.month, today.day)).order_by('date_from')
		for records in maintenance_records:
			date_from.append(records.date_from)
			date_to.append(records.date_to)
...
def deleteMaintenance(request):
	...
	try:
		...
		return JsonResponse(json_obj)
	try:
		maintenance_record = Maintenance.objects.get(serial_no=serial_no, email=email, date_from=date_from, date_to=date_to, location=location, desc=desc)
...
def retrieveOngoingJobs(request):
	...
	try:
		...
		return JsonResponse(json_obj)
	try:
		history_jobs = JobList.objects.filter(email=email, completed='0', date_from__lte =datetime.date(today.year, today.month, today.day)).order_by('date_from')
		for job in history_jobs:
			jid.append(job.JID)
			title.append(job.title)
			description.append(job.description)
...
def retrieveUpcomingJobs(request):
	...
	try:
		...
		return JsonResponse(json_obj)
	try:
		history_jobs = JobList.objects.filter(email=email, completed='0', date_from__gt=datetime.date(today.year, today.month, today.day)).order_by('date_from')
		for job in history_jobs:
			jid.append(job.JID)
			title.append(job.title)
			description.append(job.description)
...
def addJob(request):
	...
	try:
		...
		return JsonResponse(json_obj)
	try:
		job = JobList.objects.create(email=email, JID=jid, payout=payout, date_from=date_from, date_to=date_to, location=location, description=description, title=title, completed='0')
		job.save()
		json_obj = {
			'result': 'true',
			'jid' : jid
...
def addUser(request):
	...
	if role == "Owner":
		...
		role = 0
	try:
		user = User.objects.create(email=email, name=name, role=role, profile_image_url=profile_url, access_token=access_token)
		user.save()
		json_obj = {'result': 'true'}
		print (json_obj)
		return JsonResponse(json_obj)
...
def testUpload(request):
	...
	report = Reporting.objects.create(id=id, email=email, serial_no=serial_no, JID=JID, timestamp=timestamp, report_type=report_type, desc=desc, location=location)
...
def getOperatorNames(request):
    ...
    try:
        ...
        for operator in operators:
            ...
            user_job = OperatorJob.objects.filter(email=user.email)#date_from__lte= date_to, date_to_gte= date_from).count()
            for i in user_job:
            	job_date = JobList.objects.get(JID=i.JID)
            	Range = namedtuple('Range', ['start', 'end'])
            	r1 = Range(start=datetime.datetime.strptime(date_from, '%Y-%m-%d').date(), end=datetime.datetime.strptime(date_to, '%Y-%m-%d').date())
...
def insertOperatorJob(request):
    ...
    try:
        for i in range(len(operator_names)):
            ...
            operatorjob = OperatorJob.objects.create(JID=jid, email=operator_names[i], vehicle_serial_no=serial_no, vehicle_model_no=model_no, pay_received='0')
            operatorjob.save()
        json_obj = {
            'result': 'true',
        }
...
def retrieveMaintenanceUpcoming(request):
    ...
    try:
        ...
        return JsonResponse(json_obj)
    try:
        upcoming_maintenance = Maintenance.objects.filter(serial_no= serial_no, date_to__gte=datetime.date(today.year, today.month, today.day)).order_by('-date_from')
...
def retrieveMaintenanceCompleted(request):
    ...
    try:
        ...
        return JsonResponse(json_obj)
    try:
        maintenance_records = Maintenance.objects.filter(serial_no = serial_no, date_to__lte=datetime.date(today.year, today.month, today.day)).order_by('date_to')
        for records in maintenance_records:
            count += 1
...
def retrieveOpOngoJobsz(request):
    ...
    try:
        ...
        for i in JIDs:
            jid.append(i.JID)
        history_jobs = JobList.objects.filter(JID__in=jid, completed='0', date_from__lte =datetime.date(today.year, today.month, today.day)).order_by('date_from')
        for job in history_jobs:
            jid_out.append(job.JID)
            title.append(job.title)
            description.append(job.description)
...
def retrieveOpUpcomJobsz(request):
    ...
    try:
        ...
        for i in JIDs:
            jid.append(i.JID)
        history_jobs = JobList.objects.filter(JID__in=jid, completed='0', date_from__gte=datetime.date(today.year, today.month, today.day)).order_by('date_from')
        for job in history_jobs:
            jid_out.append(job.JID)
            title.append(job.title)
            description.append(job.description)
...
def addReport(request):
	...
	try:
		report = Reporting.objects.create(email=op_email, serial_no=serialno, model_no=modelno, JID=Jid, timestamp=TimeStamp,report_type=reptype,image=imgLoc,desc=Desc,location=Loc,faults=faults)
		report.save()
		json_obj = {'result': 'true'}
		print (json_obj)
		return JsonResponse(json_obj)
...
def getOperatorDetails(request):
    ...
    try:
        with connection.cursor() as cursor:
            select_query = "SELECT u.name, u.profile_image_url from volvoce.volvoce_user u where email=\'%s\';" % (str(email))
            cursor.execute(select_query)
            for row in cursor:
                user_name = row[0]
                profile_image_url = row[1]
            select_query = "SELECT v.ImgURl, v.vtype FROM volvoce.volvoce_operatorvehicle ov INNER JOIN volvoce.volvoce_vehicle v ON ov.Vtype = v.vtype WHERE ov.opEmail = \'%s\';" % (str(email))
            cursor.execute(select_query)
            for row in cursor:
...
def getJobInfo(request):
    ...
    try:
        ...
        for op in operator_job:
            ...
            vehicles.append(str(VehicleList.objects.get(serial_no=op.vehicle_serial_no, email=owner_email).serial_no) + ' - ' + str(VehicleList.objects.get(serial_no=op.vehicle_serial_no, email=owner_email).vehicle_type))
        json_obj = {
...
def addMaintenance(request):
    ...
    try:
        ...
        return JsonResponse(json_obj)
    try:
        maint = Maintenance.objects.create(serial_no=serial_no, model_no=model_no, email = email, date_from=date_from,date_to=date_to,location=location,desc=desc)
        maint.save()
        json_obj = {
            'result' : 'true'
        }
		...
		for vehicle in vehicles:
			...
			for maintenance in vehicle_maintenances:
				...
				if overlap > 0:
					availability[-1] = 'Busy'
					break
			vehicles_used_for_job = OperatorJob.objects.filter(vehicle_serial_no=vehicle.serial_no, vehicle_model_no=vehicle.model_no)
...
def addCompany(request):
    ...
    try:
        ...
        email = user.email
        user.save()
        company_details = company.objects.create(ownerEmail=email, companyName=company_name, companyAddress=company_add, no_working_days=working_days)
